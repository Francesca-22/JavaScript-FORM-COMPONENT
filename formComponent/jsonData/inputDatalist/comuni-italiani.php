...
#comune
#json
$comuniJSON = import_json('./comuni.json');

foreach ($comuniJSON as $comuneJSON) {
    $comuneJSON_nome = $comuneJSON['nome'];
    $comuneJSON_nome_lowercase = mb_strtolower($comuneJSON_nome);

    if (is_parzialmente_trovato($inputDatalistValue, $comuneJSON_nome_lowercase)) {
        echo "<option>".$comuneJSON_nome."</option>";
    }
}
